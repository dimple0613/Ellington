...
export default function SnaggingScreen({ scope }: { scope?: string }) {
  ...
  const [trade, setTrade] = useState("MEP");
  ...
  const raiseSnag = () => {
    if (!desc.trim() || !unit.trim()) { setErr("Add a unit and a short snag description"); return; }
    const newRow: SnagRow = { unit: unit.trim(), loc, trade, desc: desc.trim(), sev, contractor: "Unassigned", status: "Open", reinspect: "To schedule" };
    setRows((r) => [newRow, ...r]);
    setRaiseOpen(false);
    setDesc("");
    setErr("");
  ...
  rows.forEach((r) => { if (r.status !== "Closed" && !closed.has(rows.indexOf(r))) tradeCounts.set(r.trade, (tradeCounts.get(r.trade) || 0) + 1); });
  const TRADES: [string, number][] = [...tradeCounts.entries()];
  const maxTrade = Math.max(1, ...TRADES.map((t) => t[1]));
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr) 1.4fr", gap: 14, marginBottom: 16 }}>
        {[
          { label: "Open snags", value: String(openCount), note: "across live snags", bad: false },
          { label: "Critical", value: String(critCount), note: "blocking handover", bad: true },
          { label: "Re-inspection due", value: String(reinspCount), note: "computed live", bad: false },
          { label: "Closed this month", value: String(closedCount), note: "computed live", bad: false },
        ].map((k) => (
        ...
        <div style={{ background: "#fff", borderRadius: 20, padding: "18px 20px", boxShadow: "0 1px 3px rgba(20,22,31,.04)" }}>
          ...
          {TRADES.length ? (
            ...
            <div style={{ fontSize: 12, fontWeight: 600, color: "#9AA0AE", padding: "4px 0" }}>No open snags yet</div>
          )}
        </div>
      </div>
        ...
        <div onMouseDown={() => { setRaiseOpen(false); setErr(""); }} style={{ position: "fixed", inset: 0, background: "rgba(20,22,31,.42)", display: "grid", placeItems: "center", zIndex: 80, padding: 24 }}>
          <div onMouseDown={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 22, padding: "26px 28px", width: "100%", maxWidth: 560, boxShadow: "0 24px 60px rgba(20,22,31,.25)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 20 }}>
              <div>
                {label("Unit")}
                <input value={unit} onChange={(e) => setUnit(e.target.value)} placeholder="Unit no (e.g. WPK-T1-0114)" style={{ width: "100%", boxSizing: "border-box", height: 40, borderRadius: 12, border: "1px solid #E4E6EE", padding: "0 12px", fontSize: 12.5, fontWeight: 600, fontFamily: "monospace", outline: "none" }} />
              </div>
              <div>
                {label("Location")}
